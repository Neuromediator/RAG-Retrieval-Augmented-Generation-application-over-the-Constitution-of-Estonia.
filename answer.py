"""
RAG answer module: retrieve relevant chunks from Chroma and generate an answer
via LiteLLM. Uses OpenAI text-embedding-3-small for queries and Gemini 2.0 Flash
(OpenRouter) for completion.
"""
...
COMPLETION_MODEL = "openrouter/google/gemini-2.0-flash-001"
RETRIEVAL_K = 10
SHORT_QUERY_K = 3
MEDIUM_QUERY_K = 6
...
SYSTEM_PROMPT = """You are a helpful assistant that answers questions about the Constitution of Estonia. Use ONLY the following context from the constitution. If the answer is not in the context, say so. Do not make up details.
...
def _get_collection():
    """Return Chroma collection for docs."""
...
def _is_smalltalk_only(text: str) -> bool:
    ...
    basic_greetings = {"hi", "hello", "hey", "yo", "sup"}
