"""
Gradio chat UI for the Constitution of Estonia RAG Knowledge Worker.
Run from project root: uv run app.py
"""
import gradio as gr
...
def format_context(docs):
    ...
    for d in docs:
        ...
        lines.append(
            f"<pre style='white-space: pre-wrap;'>{d.page_content[:2000]}</pre>"
        )
    return "\n".join(lines)


def print_context_to_console(docs):
    """Print retrieved context to terminal for debugging."""
    print("\n" + "=" * 80)
...
def main():
    ...
    with gr.Blocks(title="Constitution of Estonia Assistant") as ui:
        gr.Markdown(
            "# Constitution of Estonia\nAsk anything about the Constitution of Estonia."
        )
        with gr.Row():
            with gr.Column(scale=1):
                ...
                msg = gr.Textbox(
                    placeholder="Ask about the Constitution of Estonia...",
                    show_label=False,
                )
                gr.Examples(
                    examples=[
                        ["What is the Riigikogu and what is its role?"],
